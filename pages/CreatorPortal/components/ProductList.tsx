...
interface ProductListProps {
  ...
  onRefresh?: () => void;
  onSubmit?: (id: string) => Promise<void>; // Added for factory promotion
  onCreate: () => void;
  onBack: () => void;
...
const ProductList: React.FC<ProductListProps> = ({ 
    items, categories, onEdit, onDelete, onBulkStatusChange, onBulkDelete, onRefresh, onSubmit, userRole = 'FACTORY'
}) => {
  ...
  const statusMap: Record<string, { label: string, color: string }> = {
    ...
    awaiting_review: { label: 'Pending Review', color: 'bg-amber-100 text-amber-700' },
    draft: { label: 'Draft', color: 'bg-stone-100 text-stone-500' },
    rejected: { label: 'Rejected (Needs Revision)', color: 'bg-red-100 text-red-700' }
  };
               ...
               <div className="bg-stone-900 text-white px-8 py-4 rounded-full shadow-2xl flex items-center gap-8 border border-white/10 backdrop-blur-md">
                   ...
                   {isFactory ? (
                       <button onClick={handleBulkSubmit} disabled={isBulkProcessing} className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-safety-700 hover:text-safety-600 transition-colors">
                           {isBulkProcessing ? <Loader2 size={14} className="animate-spin"/> : <FileUp size={14} />} Submit Review
                       </button>
                   ) : (
