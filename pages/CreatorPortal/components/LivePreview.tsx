          ...
          <div className="p-6 flex flex-col flex-grow bg-white">
            <div className="mb-2">
              <h3 className="font-serif text-xl text-stone-900 group-hover:text-amber-700 transition-colors line-clamp-1">
                {language === 'zh' ? (formData.name_zh || formData.name || '产品名称') : (formData.name || 'Product Name')}
              </h3>
            </div>
            <p className="text-xs text-stone-500 line-clamp-3 leading-relaxed mb-4 flex-grow">
              {language === 'zh' 
                ? (formData.description_zh || formData.description || "优质实木结构。")
            ...
            {(formData.material || formData.dimensions) && (
              <div className="text-[10px] text-stone-400 mb-4 border-t border-stone-100 pt-3 space-y-1">
                {formData.material && <div className="flex justify-between"><span>{language === 'zh' ? '材质:' : 'Mat:'}</span> <span className="text-stone-600 font-medium">{formData.material}</span></div>}
                {formData.dimensions && <div className="flex justify-between"><span>{language === 'zh' ? '尺寸:' : 'Dim:'}</span> <span className="text-stone-600 font-medium">{formData.dimensions}</span></div>}
              </div>
            )}
            ...
            <div className="pt-4 border-t border-stone-100 flex justify-between items-center mt-auto">
              <span className="text-[10px] text-stone-400 uppercase tracking-widest font-bold">
                {language === 'zh' ? '按需定制' : 'Made to Order'}
              </span>
